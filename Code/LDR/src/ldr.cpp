...
void setup() {
  Serial.begin(115200);
  Serial.println("setup bena complete");
...
void loop() {
  ...
  */
  ...
  /*
  if(value > 150){ //als opeens de waarde met meer dan 1000 afneemt, dan timer stoppen
    //off_time = millis();
    //totalTime += off_time - on_time;
    //last_value = value;
    Serial.println(off_time - on_time);
  }
  */
  ...
  if (deviceConnected) {
        ...
        Serial.println("Nog steeds geconnecteerd");
